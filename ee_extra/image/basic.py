...
def minvalue(x: ee.Image, scale: float = None):
    """Get a minimum value.
    ...
    Args:
        x (ee.Image): An ee.Image
        scale (str, optional): A nominal scale in meters of the projection
...
def maxvalue(x: ee.Image, scale: float = None):
    """Get a maximum value.
